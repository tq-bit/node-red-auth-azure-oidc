...
function assembleAzureLoginUrl(options, version = '2.0') {
	if (options.identityMetadata) {
		return options.identityMetadata;
	} else {
		return `https://login.microsoftonline.com/${options.tenant}/v${version}/.well-known/openid-configuration`;
	}
}
	...
	return {
		...
		strategy: {
			...
			options: {
				...
				identityMetadata: assembleAzureLoginUrl(options, '2.0'),
